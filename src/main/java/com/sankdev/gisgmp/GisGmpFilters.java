...
public class GisGmpFilters {
  ...
  public GisGmpFilters() {
    ...
    String edPayeePersonalAccRegExOne
      = "^(03100|03212|03222|03232|03242|03252|03262|03272|03214|03224|03234|03254).*";
   ...
   * @return {@code true} если документ ED101 должен быть отправлен в ГИС ГМП.
   */
  public boolean qualifyForGisGmp(ED101 ed101) {

    String payeeBankCorrespAcc =
      ed101.getPayee().getBank().getCorrespAcc();
    // когда нет номера корр. счета, чаще всего платеж типа
    ...
    // когда нет номера счета получателя, платеж не соответсвует
    // критериям отправки в ГИС ГМП (например, перечисление
    // заработной платы по реесту)
    if (payeePersonalAcc == null) {
      return false;
    }

    Matcher payeeBankCorrespAccMatcher = this.edPayeeBankCorrespAccPattern
      .matcher(payeeBankCorrespAcc);
  ...
  /**
   * Проверяет, что в документе типа ed101 должен быть заполнен Идентификатор плательщика (ИП) в
   * формате ///;ИП<код документа>;<номер документа>. Идентификатор плательщика должен формироваться
   * автоматически и добавляется в конец Назначения платежа. Иначе может возникать ошибка {@link
   * com.sankdev.gisgmp.GisGmpError}.
   *
