...
public class XmlUtilImpl<T> implements XmlUtil<T> {
  ...
  /**
   * Constructor creates an instance of {@link XmlUtilImpl} with contextPath of the specified
   * package. The package must contain the parameter type {@code T} class mapped to the root
   * element.
   *
   * @param typeParameterClass the class mapped to the root element of an xml file.
   * @param jaxbContextPackage the context path (the package JAXB scans to find the classes bound to
   *                           xml-schemas).
